...
export const markpubTextSchema = z.object({
  $type: z.literal('at.markpub.text').optional(),
  markdown: z.string()
});

export const markpubMarkdownContentSchema = z.object({
  $type: z.literal('at.markpub.markdown'),
  text: markpubTextSchema,
  flavor: z.enum(['gfm', 'commonmark']).optional(),
  renderingRules: z.string().optional(),
  extensions: z.array(z.string()).optional()
});

// Legacy custom embed still present on older published records. Read-only.
export const legacyMarkdownContentSchema = z.object({
...
export const documentContentSchema = z.union([
  markpubMarkdownContentSchema,
  legacyMarkdownContentSchema
]);
...
export type MarkpubMarkdownContent = z.infer<typeof markpubMarkdownContentSchema>;
